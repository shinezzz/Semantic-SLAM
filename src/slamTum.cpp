...
FRAME readFrame( int index, ParameterReader& pd,vector<string> &vstrImageFilenamesRGB,
...
int main( int argc, char** argv )
{
    ...
    string strAssociationFilename = string(argv[1]);
    ...
    FRAME currFrame = readFrame( currIndex, pd ,vstrImageFilenamesRGB,vstrImageFilenamesD); // 上一帧数据
    ...
    *******************************/
    ...
    for ( currIndex=startIndex+1; currIndex<endIndex; currIndex++ )
    {
        ...
        FRAME currFrame = readFrame( currIndex, pd ,vstrImageFilenamesRGB,vstrImageFilenamesD);
...
{
    FRAME f;
    string rgbDir   =   pd.getData("rgb_dir_tum");
    string depthDir =   pd.getData("depth_dir_tum");
    ...
    std::cout<< "filename:" << rgbDir+vstrImageFilenamesRGB[index] << std::endl;
    f.rgb = cv::imread(rgbDir+vstrImageFilenamesRGB[index],CV_LOAD_IMAGE_UNCHANGED);
    f.depth = cv::imread(depthDir+vstrImageFilenamesD[index],CV_LOAD_IMAGE_UNCHANGED);
